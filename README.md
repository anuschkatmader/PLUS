...
onetrust-custom-banner.css OneTrust admin "Custom CSS" — brand-matched banner styling
.gitlab-ci.yml             GitLab Pages deployment
```

No build step, no dependencies. Everything runs client-side.

**Imagery:** product/recipe photos are hotlinked from Pexels (free license, no
attribution required) via their images CDN, so the repo stays small and no
image assets need committing. An internet connection is required for photos to
render; each visual keeps a solid-color fallback background if a photo fails.
Brand color: PLUS green `#8DAB0A` (hover `#718908`, pale tint `#F3F6E1`).
...
## Styling the OneTrust banner

`onetrust-custom-banner.css` contains brand-matched styling (green pill
buttons, Nunito, rounded corners) for the banner, Preference Center, toggles,
and the floating cookie button. In the OneTrust admin console open your
banner/PC template → Styling → **Custom CSS** and paste the file contents.
Selectors target OneTrust's standard IDs (`#onetrust-banner-sdk`,
`#onetrust-pc-sdk`, `#onetrust-accept-btn-handler`, etc.); if your template
version differs, tweak accordingly.
